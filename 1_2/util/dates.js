...
const beginning_of_day = (date) =>
  new Date(date.getFullYear(), date.getMonth(), date.getDate());
const beginning_of_hour = (date) =>
  new Date(
    date.getFullYear(),
    date.getMonth(),
    date.getDate(),
...
const latest_week = (date) =>
  range(7).map((d) => days_later(d)(week_before(beginning_of_day(date))));
