...
const random_subset = (elements) => elements.filter((_) => Math.random() < 0.5);
...
const generate_data = (event) => [
  ...
  model.precipitation(
    random_number(precipitations),
    random_element(precipitation_types),
    event
  ),
  model.wind(
    random_number(wind_speeds),
    random_element(wind_directions),
    event
  ),
...
const generate_predictions = (event) => [
  ...
  model.precipitation_prediction(
    random_interval(precipitations),
    random_subset(precipitation_types),
    event
  ),
  model.wind_prediction(
    random_interval(wind_speeds),
    random_section(wind_directions),
    event
  ),
  model.cloud_prediction(random_interval(coverages), event),
];

const alertable = ({ type, to }) =>
  (type === "precipitation" && to >= 17) || (type === "wind speed" && to >= 36);

const generate_historic_times = (date) =>
  hours_in_latest_week(beginning_of_day(date));
const generate_historic_events = pipe(
  generate_historic_times,
  flatMap((time) => places.map((place) => model.event(time, place)))
);
const generate_historic_data = pipe(
  generate_historic_events,
  flatMap(generate_data)
);

const generate_future_times = (date) =>
  next_24_hours(hours_after(beginning_of_hour(date))(1));
const generate_future_events = pipe(
  generate_future_times,
  flatMap((time) => places.map((place) => model.event(time, place)))
);
const generate_forecast = pipe(
  generate_future_events,
  flatMap(generate_predictions)
);
const regenerate_forecast = (forecast) =>
  pipe(
    generate_future_events,
    flatMap((event) => {
      const { time, place } = event({});
      const existing_predictions = forecast.filter(
        (p) => p.time.getTime() === time.getTime() && p.place === place
      );
      const new_predictions = generate_predictions(event);
      if (existing_predictions.length === 0) return new_predictions;
      else
        return existing_predictions.map((p) => {
          const adjustment = 0.8 + Math.random() * 0.4;
          if (alertable(p) && Math.random() < 0.5)
            return Object.assign({}, p, {
              from: round_to(1)(p.from * adjustment),
              to: round_to(1)(p.to * adjustment),
            });
