...
const with_equals =
  (...keys) =>
  (o) =>
    Object.assign({}, o, { equals: equals(...keys)(o) });

const data_type = (type, unit) => (properties) =>
  Object.assign({}, properties, { type, unit });

const event = (time, place) => (properties) =>
  Object.assign({}, properties, { time, place });

const measurement = (data_type) => (object, event) =>
  Object.assign(
...
const temperature_prediction = ({ from, to }, event) =>
  prediction_equals(temperature_measurement({ from, to }, event));

const wind = (value, direction, event) =>
  wind_measurement({ value, direction }, event);
const wind_prediction = ({ from, to }, directions, event) =>
  with_equals(prediction_keys.concat("directions"))(
    wind_measurement({ from, to, directions }, event)
  );

const precipitation = (value, precipitation_type, event) =>
  precipitation_measurement({ value, precipitation_type }, event);
const precipitation_prediction = ({ from, to }, precipitation_types, event) =>
  with_equals(prediction_keys.concat("precipitation_types"))(
    precipitation_measurement({ from, to, precipitation_types }, event)
  );

const cloud = (value, event) => cloud_measurement({ value }, event);
const cloud_prediction = ({ from, to }, event) =>
  prediction_equals(cloud_measurement({ from, to }, event));
  ...
  const create_alert = (id, severity, prediction) => {
    const matches = (other_prediction) => {
      return (
        prediction &&
        other_prediction &&
        other_prediction.time &&
        other_prediction.place &&
        other_prediction.type &&
    ...
    return with_equals(
      ...
      "severity",
      "prediction"
    )({ id, severity, prediction, matches, cancelled, updated });
  };
  if (
    id < 1 ||
    severity < 1 ||
